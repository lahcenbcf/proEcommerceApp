...
const Navbar = () => {
  ...
  return (
    ...
    {/* menuBar icon */}
    <div className="absolute left-2 mr-10 z-20 bg-white text-secondary p-3 rounded-full md:hidden" onClick={()=>setToggleBar(!toggleBar)}>
    {
    ...
    {/* search box */}
    {
...

        {
          ...
          <BsFillCartFill size={20} className="opacity-70 hover:opacity-100" />
  ...
  <ul className="p-2 shadow menu dropdown-content z-[1] bg-base-100 rounded-box w-52 text-black font-bold">
    <li><Link to={"/admin"} className="uppercase">users</Link></li>
    <li><Link to={"/admin/productList"} className="uppercase">products</Link></li>
    <li><Link to={"/admin/orderList"} className="uppercase">orders</Link></li>
  </ul>
</details>
        }
</li>
<li>
        <Link to={"/signIn"} className="flex items-center gap-2">
            <BiSolidUser size={20} className="opacity-70 hover:opacity-100"/>
            <h3 className="opacity-70 hover:opacity-100 uppercase ">{userInfo._id ? "logout" : "signIn"}</h3>
